...
LowLoop is an asynchronous event-driven server that ties into LowEvent to create and send events from the request layer right through to the application and data layers. Finally you can see and track events through every step of your application. Simply add `observe 'path/:id'` to a LowNode and now it will be called every time a request is made to this route.
